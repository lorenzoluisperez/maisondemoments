...
export function StudioWorkspace() {
  ...
  return <AppShell area="studio">
    ...
    <section className="studio-editor"><div className="editor-toolbar"><div><p className="workspace-kicker">{workspace.order.jobNumber}</p><h2>{selectedOrder?.customerName}</h2></div><div><button className="icon-button" aria-label="Undo" disabled={saveState === "saving"} onClick={() => restore("undo")}><Undo2 /></button><button className="icon-button" aria-label="Redo" disabled={saveState === "saving"} onClick={() => restore("redo")}><Redo2 /></button><span className={saveState === "saved" ? "save-state saved" : "save-state"}>{saveState === "saved" ? <CheckCircle2 /> : <RefreshCw />}{saveStateLabel(saveState)} · r{workspace.draft.revision}</span><button className="workspace-button" disabled title="Review creation is implemented in Phase 5">Create review</button></div></div>
