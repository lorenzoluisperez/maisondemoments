...
import { correctHouseholdRsvp } from "@/lib/guests/service";
...
export async function POST(request: Request, { params }: { params: Promise<{ orderId: string; groupId: string }> }) {
  ...
  try { const actor = await requireCurrentActor(); const values = await params; return NextResponse.json(await correctHouseholdRsvp(actor, values.orderId, values.groupId, await readBoundedJson(request)), { headers: { "Cache-Control": "private, no-store" } }); }
