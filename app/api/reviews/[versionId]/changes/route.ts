...
import { requestReviewChanges } from "@/lib/reviews/service";
...
export async function POST(request: Request, { params }: { params: Promise<{ versionId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ review: await requestReviewChanges(actor, versionId, await readBoundedJson(request)) }, { headers: { "Cache-Control": "private, no-store" } });
  } catch (error) {
