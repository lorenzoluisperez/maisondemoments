...
import { changeInvitationAvailability } from "@/lib/reviews/service";
...
export async function POST(request: Request, { params }: { params: Promise<{ invitationId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ history: await changeInvitationAvailability(actor, invitationId, await readBoundedJson(request)) }, { headers: { "Cache-Control": "private, no-store" } });
  } catch (error) {
